...
from triangle_matcher import (
    triangle_predict_from_bbox,
    triangle_predict_from_image,
    decide_yolo_triangle_result,
)
            ...
            stars.append(
                {
                    "x": float(center_x + x1),
                    "y": float(center_y + y1),
                    "brightness": float(region_pixels.mean()),
                    "area": area,
                }
            )
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        show_detection_table = st.checkbox("Show detection details table", value=True)
        st.markdown("---")
    ...
    # -----------------------------------
    # Process YOLO detections + Triangle verification
    # -----------------------------------
    for result in results:
        for box, class_id, score in zip(
            ...
            yolo_label = str(result.names.get(class_id, class_id))
            yolo_conf = float(score)

            triangle_result = triangle_predict_from_bbox(
                image_bgr=image_bgr,
                bbox=bbox,
                max_stars=max_stars,
            ...
            final_result = decide_yolo_triangle_result(
                yolo_label=yolo_label,
                yolo_confidence=yolo_conf,
                triangle_label=triangle_result["label"],
                triangle_score=triangle_result["score"],
            )
            ...
            detected_sign = final_label.lower()
            ...
            zodiac_result = get_zodiac_result(
                detected_sign,
                birth_month,
                birth_day,
            )
            ...
            annotated = draw_detection_box(
                annotated,
                bbox,
                f"{final_label} | {final_result['method']}",
            )

            star_points = detect_star_keypoints(
                image_bgr,
                bbox,
                max_stars,
                brightness_percentile,
            ...
            detection_rows.append(
                {
                    "Detection": len(detection_rows) + 1,
                    "YOLO": yolo_label,
                    "Triangle": triangle_result["label"],
                    "Final": final_label,
                    "Method": final_result["method"],
                    "YOLO Confidence": round(yolo_conf, 3),
                    "Triangle Score": round(triangle_result["score"], 3),
                    "Detected star centers": len(star_points),
                }
            )

    # -----------------------------------
    # If YOLO found nothing, use Triangle only
    # -----------------------------------
    if len(detection_rows) == 0:
        triangle_result = triangle_predict_from_image(
            image_bgr=image_bgr,
            max_stars=max_stars,
            brightness_percentile=brightness_percentile,
            min_area=int(min_star_area),
        ...
        if triangle_result["label"] is not None:
            final_label = triangle_result["label"]
            ...
            st.success(
                f"Triangle Matching detected: {final_label}"
            )
            ...
            detection_rows.append(
                {
                    "Detection": 1,
                    "YOLO": "No detection",
                    "Triangle": final_label,
                    "Final": final_label,
                    "Method": "Triangle only",
                    "YOLO Confidence": 0.0,
                    "Triangle Score": round(triangle_result["score"], 3),
                    "Detected star centers": triangle_result["num_points"],
                }
            )
    ...
    if show_detection_table and detection_rows:
        st.markdown("### Detection Details")
        st.dataframe(detection_rows, use_container_width=True)
    ...
    if zodiac_result is not None:
        ...
        with st.expander("💕 Compatible Signs"):
            st.write(
                ", ".join(
        ...
        with st.expander("🔮 Daily Guidance"):
            st.write(
                f"**Lucky Color:** "
